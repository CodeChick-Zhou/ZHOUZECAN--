...
class Background(object):
    ...
    def init_frame(self):
        File.readfile()
        print(File.curpathindex)
        self.frame.setStyleSheet("QWidget#Frame{border-image:url("+File.path[File.curpathindex-1]+");"
            +" border-top-right-radius:10px;"+"border-bottom-right-radius:10px;"+"border-top-left-radius:10px;"
        ...
        self.backbutton.clicked.connect(self.writefile)
        ...
        # palette = QPalette()
        # palette.setBrush(self.frame.backgroundRole(), QBrush(QPixmap("../images/screen7.jpg").scaled(self.frame.size())))
        ...
        for i in range(0, len(File.stars)):
            # 获得图片路径
            # 生成图像objPixmap
            objPixmap = QPixmap(File.path[i])
            # 生成QListWidgetItem对象(注意：其Icon图像进行了伸缩[96 * 96])---scaled函数
            pItem = QListWidgetItem(QIcon(objPixmap.scaled(QSize(self.W_ICONSIZE, self.H_ICONITEM))),
                                    "⭐ : " + File.stars[i])
    ...
    def check(self,index):
        ...
        if int(File.curstars) > int(File.stars[r]):
            File.curpathindex = r + 1
            self.frame.setStyleSheet("QWidget#Frame{border-image:url(" + File.path[r] + ");"
                                     + " border-top-right-radius:10px;" + "border-bottom-right-radius:10px;" + "border-top-left-radius:10px;"
                                     + "border-bottom-left-radius:10px;}")
            # palette = QPalette()
